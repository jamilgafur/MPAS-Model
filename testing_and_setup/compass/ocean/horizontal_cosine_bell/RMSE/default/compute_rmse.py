...
# sliceTag is the time slice, assumes 1 hour output and same for all cases
    ...
    for i, line in enumerate(temp):
        if "config_horizontal_cosine_bell_lat_center" in line: 
            ii = line.find('=')+1
            latCent = float(line[ii:])
        if "config_horizontal_cosine_bell_lon_center" in line: 
            ii = line.find('=')+1
            lonCent = float(line[ii:])
        if "config_horizontal_cosine_bell_radius" in line: 
            ii = line.find('=')+1
            radius = float(line[ii:])
        if "config_horizontal_cosine_bell_psi0" in line: 
            ii = line.find('=')+1
            psi0 = float(line[ii:])
        if "config_horizontal_cosine_bell_vel_pd" in line: 
            ii = line.find('=')+1
            pd = float(line[ii:])
